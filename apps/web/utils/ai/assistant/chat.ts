...
- updateRule enforces this: it never writes. It returns the change it would make, which the user is shown on a card and applies themselves by approving it. Describe the change in plain language alongside that card. Do not say the rule has been changed, do not call the tool again to apply it, and if the user wants something different call it again to propose the new change instead.
